...
@WebServlet(name = "seeRequest", urlPatterns = {"/seeRequest"})
public class seeRequest extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        PrintWriter out = response.getWriter();
         ...
         try{
            ...
            String q1 = new String("SELECT * FROM  userinfo, friends where userinfo.user_name=friends.host and friends.guest='"
                    +loginServlet.userid+"'"+"and friends.status='request'");         
        ...
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Give your reponse!</h1>");
        out.println("<ul>");
       try{
           while(rs.next())
           {
             out.println("<form method = \"post\" action=\"dealResponse\" id=\"send\">"+"<table>"+"<tbody>"+"<tr>");
             ...
             out.println("<td>"+rs.getString("name")+"&nbsp"+"sends you a request."+"</td>");
              out.println("<td>&nbsp Basic information about this person: hobby:</td>");
             out.println("<td>"+rs.getString("hobby")+"</td>");
             
             out.println("<td><input type=\"submit\" name=\"req\" id=\"req\" value=\"send your response\""+"/>"+
            "<select name=\"choice\"><option value=\"yes\">yes</option><option value=\"no\">no</option></select>"+"</td>");
