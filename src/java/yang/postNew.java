...
@WebServlet(name = "postNew", urlPatterns = {"/postNew"})
public class postNew extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Boolean valid = false;
        String uname=null;
        String n=request.getParameter("news");
        try{Class.forName("org.apache.derby.jdbc.ClientDriver");
            ...
            String q = new String("select * from userinfo where user_name="+"'"+loginServlet.userid+"'");                     
            rs= st.executeQuery(q);                    
                while(rs.next()){          
                uname=rs.getString("name");                
                }
                valid=true;
            String q2 = new String("insert into news (N) values('"+uname+":"+n+"')");  
            st.execute(q2);             
            }
        ...
        if(valid==true){
            cb.setReminder2("Your breaking news has been post");
